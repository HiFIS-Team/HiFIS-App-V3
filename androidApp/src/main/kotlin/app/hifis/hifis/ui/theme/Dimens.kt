...
object Dimens {
    ...
    /**
     * 굴러가는 화면이 **맨 아래에 더 두어야 하는 여백** — 떠 있는 AI 단추가 덮는 만큼
     *
     * 단추는 본문 위에 떠 있어서 맨 아래 줄을 가린다. 글자만 가리면 굴려서 읽으면 그만인데,
     * **누르는 것을 가리면 못 누른다** — 업무 칩 격자의 마지막 오른쪽 칸 `+` 가 정확히
     * 그 자리였다 (재서 확인했다). 이만큼 더 두면 그 줄을 단추 위로 올릴 수 있다.
     *
     * **iOS 에는 이 값이 필요 없다.** 거기는 AI 자리가 탭바가 그리는 동그라미라
     * 본문 위에 뜨는 것이 없다.
     */
    val aiChatClear = aiChatButton + aiChatMargin * 2
}
